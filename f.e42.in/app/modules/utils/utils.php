...
function init_shop_constants() {
	...
	if( isset($_SESSION['admin']['shop']['shop_id']) ) {
        ...
        if ($DOMAIN == 'localhost' || $DOMAIN == '192.168.1.20'  ) {
                define('SHOP_CDN',LOCAL_CDN_BASE."$domain/");
				define('SHOP_BASEURL',"$protocol//$DOMAIN/cn/fueltracker");
        } else {
...
function has_user_permission($a='',$b='',$c='',$d='') {
	return true;
}

function init_travel($domain) {

	LOG_MSG('INFO',"init_travel(): START domain=[$domain] ");

	if ( $domain == "localhost" ) {

                // Get either nfc tag or the imei
		$nfc_tag_id=get_arg($_GET,"nfc_tag_id");
		$imei=get_arg($_GET,"imei");


		// Validate the args nfc_tag_id and imei
		if ( !validate("NFC Tag Id",$nfc_tag_id,0,45,"varchar") ||
				 !validate("IMEI no",$imei,0,45,"varchar") ) {
				$json_response['message']=$ERROR_MESSAGE;
				echo json_encode($json_response);
				exit;
		}
		...
		if ( $nfc_tag_id != "" ) {
			if ( ($travel_id=db_get_list('LIST','travel_id','tTagSticker',"nfc_tag_id='$nfc_tag_id'")) === false ) {
			...
			if ( ($travel_id=db_get_list('LIST','travel_id','tSupervisor',"imei='$imei'")) === false ) {
				LOG_MSG('ERROR',"init_travel(): Error getting travel_id for domain=[$domain] ");
				$json['message']="Invalid Card";
				echo json_encode($json);
				exit;
		...
		if ( $travel_id === '' ) {
			...
			echo json_encode($json_response);
			LOG_MSG('ERROR',"init_travel(): Travel ID not found");
			exit;
		}
