<?php  
	...
	while ( ($item = fgetcsv($handle, 65535, ',','"')) !== FALSE ) {
		...
		$client_name=		clean_csv_string(get_arg($item,2));
		$sticker_no=		clean_csv_string(get_arg($item,3));
		echo "[$sticker_no]"; 

		//upload the details into csv if sticker no is not empty
		if( $sticker_no == "" ) continue;

		if( strlen($sticker_no) == 1 ) {
			$sticker_no="MEG00".$sticker_no;
		}
		else if( strlen($sticker_no) == 2 ) {
			$sticker_no="MEG0".$sticker_no;
		}
		else {
			$sticker_no="MEG".$sticker_no;
		}
