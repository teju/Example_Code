...
function db_shopsetting_intercom_select($shop_id) {
	...
	$resp=execSQL("SELECT 
					...
					WHERE
						shop_id = $shop_id 
						AND name IN ('subscription_end_date','subscription_is_trial','subscription_is_renewed','max_no_of_products','account_manager') "
					,array(), 
					false);
...
//AppSync INSERT
function db_appsync_insert( $table_name,
							$primary_id,
							$status,
							$travel_id) {
	$param_arr=_init_db_params();

	LOG_MSG('INFO',"db_appsync_insert(): START { 
							table_name=[$table_name],
							primary_id=[$primary_id],
							status=[$status] \n}");

	// Add params to params_arr
	$param_arr=_db_prepare_param($param_arr,"s","table_name",$table_name);
	$param_arr=_db_prepare_param($param_arr,"s","primary_id",$primary_id);
	...
	LOG_MSG('INFO',"db_appsync_insert(): END");
	return $resp;
}

// Update is_sync to 0 after inserting a row in table tappsync
function db_is_sync_supervisor_update(	
							$is_sync) {
	$param_arr=_init_db_params();
