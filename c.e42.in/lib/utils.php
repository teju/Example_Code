...
/*                          SEND EMAIL                                */
/**********************************************************************/
function send_email($to,$from,$cc='',$bcc='',$subject,$message,$attachments_arr='',$plain_message='') {
	...
	if (EMAILER_HOST == 'LOCAL' ) {
		...
		if ($attachments_arr) {
			...
			$attachment = Swift_Attachment::newInstance($attachments_arr[0]['data'], $attachments_arr[0]['filename'], 'application/pdf');
...
function LOG_MSG($level,$msg)
{
	...
	// LOGGED IN
	if ( isset($_SESSION['admin']['logged_in']) && $_SESSION['admin']['logged_in'] === 1 ) {	//SNT0098: To prevent cross session issue with shop
		$utype='N';
		$id="<".$utype.":".$_SESSION['admin']['user_id'].":".$_SESSION['admin']['email_id'].">";
	// NOT LOGGED IN
	} else {
		$id="<".get_ip().":GUEST>";
	}
